...
The first thing I tackled was pipelines. Instead of fragile, thousand-line scripts, I use simple project layouts and pipeline managers so analyses can be rerun without guesswork — not just by me, but by collaborators down the line. 

My go-to is the **targets** package in R, which can reproduce an entire project, including manuscripts, with a single command.

Pipelines don’t fix everything, so I lock environments. **Docker** standardizes the system across machines, and tools like **renv** freeze package versions for your project. That way others — even years later — can run the code exactly as I did.
...
*The best thing is, these practices are deliberately general.* You can do the same workflow using Python, conda, and snakemake, for example. And the same pattern — modular steps, QC checkpoints, and versioned environments — can be used for ecology, genomics (FASTQ/VCF), and even metadata/data management plans basics. 
...
IGB is one of the world's leading freshwater research centers, with real reach across science and policy. That makes it an ideal place to standardize robust, open workflows that others can learn from and adopt.
...
I'm happy to try to answer any and all questions.

Oh, just a final note: this talk was made using Quarto and all files are available on GitHub!
